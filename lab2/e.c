    ...
    if (ch1==-1) {
        ...
        if (ch2==-1) {
            ...
            while(1) {
                ...
                // printf("%c, %c\n",(guess1[0]),(guess2[0]));
                printf("Player 1 guessed: %s.\n", colors[guess1[0]-'0']);
                printf("Player 2 guessed: %s.\n\n", colors[guess2[0]-'0']);
                ...
                if(guess1[0]==choice && guess2[0]==choice) {
                    ...
                    if (guess1[0]==choice) {
                        kill(ch1, SIGUSR1); // child 1 won
                        kill(ch2, SIGUSR2); // child 2 lost
                        ...
                        kill(ch1, SIGUSR2); // child 1 lost
                        kill(ch2, SIGUSR1); // child 2 won
                        ...
                        kill(ch1, SIGUSR1); // child 1 won
                        if (ch2_score>=50){
                            kill(ch2, SIGUSR1); // child 2 won too
                        ...
                        kill(ch2, SIGUSR1); // child 2 won
                        if (ch1_score>=50){
                            kill(ch1, SIGUSR1); // child 1 won too
                            printf("Both the players have won.\n");
                            printf("Current Scores-\nP1: %d, P2: %d\n\n", ch1_score, ch2_score);
                        }
                        else {
