...
for n in N:
    ...
    for i in range(10):
        ...
        generator = subprocess.Popen(["./gen", str(n), "4"], stdout=subprocess.PIPE)
    ...
    for m in trial_results.keys():
        ...
        response /= 10
        turnaround /= 10
        results[n][m] = [waiting, turnaround, response]

for n in results:
    print(f"Number of processes: {n}")
    for m in methods:
        print(m,":",f"Waiting: {results[n][m][0]:.2f}", f"Turnaround: {results[n][m][1]:.2f}", f"Response: {results[n][m][2]:.2f}")
    print()
...
y5 = []
for n in N:
    y1.append(results[n]["FCFS"][1])
    y2.append(results[n]["NSJF"][1])
    y3.append(results[n]["PSJF"][1])
    y4.append(results[n]["RR"][1])
    y5.append(results[n]["P"][1])
    # print(results[n]["FCFS"][0], end = " ")

plt.xlabel("Number of Processes (N)")
plt.ylabel("Average Turnaround Time (ATT)")
plt.plot(x, y1)
plt.plot(x, y2)
plt.plot(x, y3)
plt.plot(x, y4)
plt.plot(x, y5)
plt.legend(["FCFS", "NSJF", "PSJF", "RR", "P"])
