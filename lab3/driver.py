...
def printer(results, title, N):
    ...
    for i, t in enumerate(times):
        ...
        plt.legend(loc='best')
        plt.savefig(f"Results/{title}_{t}_Time.png", dpi=300)
        plt.clf()
        # plt.show()
# loop over N values
...
def compiler():
    ...
    gen_compile = subprocess.Popen(
        "gcc -o gen generator.c -lm".split(' '), stdout=subprocess.PIPE)
    ...
    sim_compile = subprocess.Popen(
        "gcc -o sim simulate.c".split(' '), stdout=subprocess.PIPE)
...
def processor():
    ...
    for n in N:
        os.makedirs(f'Results/{n}_processes', exist_ok=True)
        ...
        for m in methods:
            trial_results[m] = {}
        for i in range(10):
            os.makedirs(f'Results/{n}_processes/trial_{i+1}', exist_ok=True)
            # first generate the data
            generator = subprocess.Popen(
                ["./gen", str(n), "2"], stdout=subprocess.PIPE)
            res, err = generator.communicate()
            with open(f"Results/{n}_processes/trial_{i+1}/data.txt", "w") as f:
                f.write("Arrival_time,Priority,CPU_burst_time\n")
                f.write(res.decode())
            sleep(1)
            ...
            for m in methods:
                ...
                simulator = subprocess.Popen(
                    ["./sim", m], stdout=subprocess.PIPE)
                res, err = simulator.communicate()
                with open(f"Results/{n}_processes/trial_{i+1}/{m}.txt", "w") as f:
                    f.write(res.decode())
    ...
    for n in avg_results:
        ...
        for m in methods:
            print(
                m, ":", f"\tWaiting: Avg={avg_results[n][m][0]:.2f} Max={max_results[n][m][0]:.2f} Min={min_results[n][m][0]:.2f}")
            print(
                f"\tTurnaround: Avg={avg_results[n][m][1]:.2f} Max={max_results[n][m][1]:.2f} Min={min_results[n][m][1]:.2f}")
            print(
                f"\tResponse: Avg={avg_results[n][m][2]:.2f} Max={max_results[n][m][2]:.2f} Min={min_results[n][m][2]:.2f}")
        print()
...
if __name__ == '__main__':
    ...
    processor()
    print("--------------------------------------------")
    print(r"All the compiled results & graphs can be found in the Results folder.")
